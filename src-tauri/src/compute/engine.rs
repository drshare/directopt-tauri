...
use crate::compute::economics::{check_constraints, compute_economics, fitness, ConstraintStatus, EconResult};
...
fn evaluate_config(
    ...
    let fit = fitness(&econ, &cons, &totals, &params.objective);
...
pub fn run_compute(
    ...
    let (best_genes, best_fitness) = ga::optimize(
        bounds,
        &params.ga,
        &evaluate,
        &on_generation,
...
pub fn build_payload(
    ...
    ComputeResultPayload {
        best: BestOut {
            ...
            ess_kwh,
            ess_kw,
            fitness: if best_fitness.is_finite() { best_fitness } else { f64::MAX },
        },
        headline,
        invest,
        opex,
...
fn build_sensitivity(
    ...
    let eval = |wind: f64, pv: f64, ess: f64| -> (f64, ConstraintStatus) {
        let (_, econ, cons, fit) = evaluate_config(wind, pv, ess, params, curves);
        let _ = econ;
        (fit, cons)
    };
